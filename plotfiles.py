...
for i, process in enumerate(lines):
    ...
    max_time = max(
        max([line[3] for line in process]), max([line[1] for line in process])
    )
    ...
    ondisk_xs = [
        [line[1] - min_time, (line[3] if line[3] > 0 else max_time) - min_time]
        for line in process
    ]
    ...
    legendLabels = legendLabels[
        (len(active_color) - 1) if len(active_color) > 1 else 0 :
    ]
...
plt.legend(
    handles=[
        plt.Line2D([0], [0], linewidth=3, color=c, label=legendLabels[process])
